...
QVariant QSystemLocalePrivate::dayName(int day, QLocale::FormatType type)
{
    ...
    GetDateFormat(lcid, 0, &st, type == QLocale::LongFormat ? L"ddMMMM" : L"ddMMM" , buf, 255);
...
QVariant QSystemLocalePrivate::monthName(int month, QLocale::FormatType type, bool standalone)
{
    ...
    return getLocaleInfo(type == QLocale::LongFormat ? long_month_map[month - 1] : short_month_map[month - 1]);
}
