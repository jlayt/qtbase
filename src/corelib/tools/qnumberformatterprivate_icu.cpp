...
static ULocDataLocaleType qt_localeTypeToIcuType(QLocale::LocaleCodeType type)
{
    switch (type) {
...
static UNumberFormatStyle qt_numberFormatToIcuFormat(QNumberFormatter::NumberStyle style)
{
    switch (style) {
...
static UNumberFormatSymbol qt_numberSymbolToIcuSymbol(QNumberFormatter::NumberSymbol symbol)
{
    switch (symbol) {
...
static UNumberFormatAttribute qt_numberAttributeToIcuAttribute(QNumberFormatter::NumberAttribute attribute)
{
    switch (attribute) {
...
static UNumberFormatPadPosition qt_numberAttributeToIcuAttribute(QNumberFormatter::PadPosition pos)
{
    switch (pos) {
...
static QNumberFormatter::PadPosition qt_icuPositionToPadPosition(UNumberFormatPadPosition pos)
{
    switch (pos) {
...
static UNumberFormatRoundingMode qt_roundingModeToIcuMode(QNumberFormatter::RoundingMode mode)
{
    switch (mode) {
...
static QNumberFormatter::RoundingMode qt_roundingModeToIcuMode(UNumberFormatRoundingMode mode)
{
    switch (mode) {
...
static QLocale::ParsingMode qt_unum_parsingMode(UNumberFormat *nf)
{
...
static QString qt_unum_toPattern(UNumberFormat *nf)
{
    int32_t size = 50;
    QString result(size, Qt::Uninitialized);
    UErrorCode status = U_ZERO_ERROR;
...
static void qt_unum_applyPattern(UNumberFormat *nf, const QString &value)
{
    UErrorCode status = U_ZERO_ERROR;
...
static void qt_setPadding(UNumberFormat *nf, qint32 width, const QString &padding,
...
static void qt_setRounding(UNumberFormat *nf, QNumberFormatter::RoundingMode mode, double increment)
{
...
static QString qt_unum_formatInt32(UNumberFormat *nf, qint32 from)
{
    int32_t size = 30;
    QString result(size, Qt::Uninitialized);
    UErrorCode status = U_ZERO_ERROR;
...
static QString qt_unum_formatInt64(UNumberFormat *nf, qint64 from)
{
    int32_t size = 60;
    QString result(size, Qt::Uninitialized);
    UErrorCode status = U_ZERO_ERROR;
...
static QString qt_unum_formatDoubleCurrency(UNumberFormat *nf, double from, QString currencyCode)
{
    int32_t size = 100;
    QString result(size, Qt::Uninitialized);
    UErrorCode status = U_ZERO_ERROR;
...
static UNumberFormat *qt_parseFormatter(UNumberFormat *mnf, QLocale::ParsingMode mode)
{
                                                 ...
                                                 const QString &localeCode)
{
    ...
    if (style == QNumberFormatter::DecimalPatternStyle || style == QNumberFormatter::RuleBasedStyle) {
        ...
        m_nf = unum_open(qt_numberFormatToIcuFormat(style),
                         reinterpret_cast<const UChar *>(pattern.data()), pattern.size(),
                         localeCode.toUtf8(), NULL, &m_status);
    } else {
        //unum_open(style, pattern, patternLength, locale, parseErr, status)
        m_nf = unum_open(qt_numberFormatToIcuFormat(style), NULL, -1, localeCode.toUtf8(), NULL, &m_status);
    }
...
QList<QNumberFormatter::NumberStyle> QNumberFormatterPrivate::availableNumberStyles();
{
    ...
    list << QNumberFormatter::DecimalStyle
         ...
         << QNumberFormatter::OrdinalStyle
         << QNumberFormatter::DurationStyle
         << QNumberFormatter::DecimalPatternStyle
         << QNumberFormatter::RuleBasedStyle;

    if (U_ICU_VERSION_SHORT >= "48") {
        list << QNumberFormatter::CurrencyCodeStyle
                QNumberFormatter::CurrencyNameStyle;
    }
