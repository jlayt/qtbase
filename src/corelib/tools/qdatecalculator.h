...
#ifndef QDATECALCULATOR_H
#define QDATECALCULATOR_H
...
class QDateCalculatorPrivate;
class QDate;

class Q_CORE_EXPORT QDateCalculator
{
public:
    explicit QDateCalculator(QLocale::CalendarSystem calendar = QLocale::DefaultCalendar);
    ~QDateCalculator();

    QLocale::CalendarSystem calendarSystem() const;
    void setCalendarSystem(QLocale::CalendarSystem calendar);
    ...
    void ymd(const QDate &date, int *year, int *month, int *day) const;
...
private:
    QDateCalculatorPrivate * const d;
};
